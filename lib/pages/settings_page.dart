...
class _SettingsPageState extends State<SettingsPage> {
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          final double cardWidth = constraints.maxWidth > 900 ? 900 : constraints.maxWidth;
          return Center(
  ...
  bool _matchQuery(String keywords) {
    if (_searchQuery.isEmpty) return true;
    return keywords.toLowerCase().contains(_searchQuery.toLowerCase());
  }
  ...
  Widget _sectionCard(Widget header, List<Widget> children) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 16),
      child: Card(
        ...
        child: Padding(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[header, const SizedBox(height: 12), ...children],
          ),
        ),
      ),
    );
  ...
  Widget _aiServiceCard() {
    if (!_matchQuery('ai 服务 api base url key 模型 连接')) {
      return const SizedBox.shrink();
    }
    return _sectionCard(
      _sectionHeader('AI 服务', Icons.memory, subtitle: 'API 连接与模型选择'),
      <Widget>[
        TextField(
  ...
  Widget _modelSection() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        Text('模型选择', style: Theme.of(context).textTheme.titleSmall?.copyWith(
          fontWeight: FontWeight.w600,
        )),
        const SizedBox(height: 8),
        Wrap(
        ...
        if (_modelsError != null) ...[
          const SizedBox(height: 8),
          Text(_modelsError!, style: TextStyle(color: Theme.of(context).colorScheme.error)),
        ],
        const SizedBox(height: 8),
        if (_models.isEmpty)
          Padding(
          ...
          Column(
            children: _models.map((String model) {
              final bool selected = model == _selectedModel;
              return ListTile(
                dense: true,
  ...
  Widget _conversationCard() {
    if (!_matchQuery('对话 策略 提示词 推进 沉浸 字数 灵感 摘要 重说 发送')) {
      return const SizedBox.shrink();
    }
    return _sectionCard(
      _sectionHeader('对话与策略', Icons.chat_bubble_outline, subtitle: '提示词、灵感、摘要及发送策略'),
      <Widget>[
        _promptStrategyGroup(),
        const Divider(),
        _inspirationSwitch(),
        const Divider(),
        _summaryGroup(),
  ...
  Widget _promptStrategyGroup() {
    final ts = Theme.of(context).textTheme;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        Text('提示词策略', style: ts.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
        const SizedBox(height: 12),
        Text('推进策略', style: ts.bodyMedium?.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant)),
        const SizedBox(height: 8),
        Row(
          children: <Widget>[
            Expanded(
              child: ChoiceChip(
                ...
                onSelected: (bool s) {
                  if (s) {
                    setState(() => _promptStrategy = _promptStrategy.copyWith(advance: AdvanceStrategy.forced));
            ...
            Expanded(
              child: ChoiceChip(
                ...
                onSelected: (bool s) {
                  if (s) {
                    setState(() => _promptStrategy = _promptStrategy.copyWith(advance: AdvanceStrategy.free));
                    _savePromptStrategySettings();
                  }
                },
              ),
        ...
        const SizedBox(height: 12),
        Text('沉浸策略', style: ts.bodyMedium?.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant)),
        const SizedBox(height: 8),
        Row(
          children: <Widget>[
            Expanded(
              child: ChoiceChip(
                ...
                onSelected: (bool s) {
                  if (s) {
                    setState(() => _promptStrategy = _promptStrategy.copyWith(immersion: ImmersionStrategy.restrained));
                    _savePromptStrategySettings();
                  }
                },
              ),
            ...
            Expanded(
              child: ChoiceChip(
                ...
                onSelected: (bool s) {
                  if (s) {
                    setState(() => _promptStrategy = _promptStrategy.copyWith(immersion: ImmersionStrategy.strong));
                    _savePromptStrategySettings();
                  }
                },
              ),
        ...
        const SizedBox(height: 12),
        Text('字数控制', style: ts.bodyMedium?.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant)),
        const SizedBox(height: 8),
        Row(
          children: <Widget>[
            Expanded(
              child: ChoiceChip(
                ...
                onSelected: (bool s) {
                  if (s) {
                    setState(() => _promptStrategy = _promptStrategy.copyWith(length: LengthStrategy.strict));
                    _savePromptStrategySettings();
                  }
                },
              ),
            ...
            Expanded(
              child: ChoiceChip(
                ...
                onSelected: (bool s) {
                  if (s) {
                    setState(() => _promptStrategy = _promptStrategy.copyWith(length: LengthStrategy.unlimited));
                    _savePromptStrategySettings();
                  }
                },
              ),
  ...
  Widget _inspirationSwitch() {
    return SwitchListTile(
      ...
      onChanged: (bool v) {
        setState(() => _inspirationIncludeSummary = v);
  ...
  Widget _securityCard() {
    if (!_matchQuery('安全 隐私 验证 生物识别 锁')) {
      return const SizedBox.shrink();
    }
    return _sectionCard(
      _sectionHeader('安全与隐私', Icons.lock_outline, subtitle: '生物识别验证保护'),
      !_authAvailable
          ? <Widget>[
              Padding(
                padding: const EdgeInsets.symmetric(vertical: 8),
          ...
          : <Widget>[
              ...
              SwitchListTile(
                ...
                onChanged: (bool v) {
                  setState(() => _requireAuthForArchive = v);
                  _saveAuthSettings();
                },
              ),
            ],
  ...
  Widget _appearanceCard() {
    if (!_matchQuery('外观 图标 动画 引导 oobe 开场')) {
      return const SizedBox.shrink();
    }
    return _sectionCard(
      _sectionHeader('外观与体验', Icons.palette_outlined, subtitle: '应用图标、动画及引导流程'),
      <Widget>[
        _appIconGroup(),
        const Divider(),
        _splashGroup(),
  ...
  Widget _appIconGroup() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        Text('应用图标', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
        const SizedBox(height: 4),
        ...
        Wrap(
          spacing: 16,
          runSpacing: 12,
          children: AppIconService.availableOptions.map((AppIconOption option) {
            return ChoiceChip(
              selected: _selectedIconKey == option.key,
              onSelected: _androidSupported ? (_) => _selectIcon(option) : null,
              label: Row(
                mainAxisSize: MainAxisSize.min,
                children: <Widget>[
                  ClipRRect(
                    borderRadius: BorderRadius.circular(8),
                    child: Image.asset(option.assetPath, width: 36, height: 36,
    ...
    return Column(
      ...
      children: <Widget>[
        SwitchListTile(
          ...
          onChanged: (bool v) {
            setState(() => _showSplashAnimation = v);
            _saveSplashSettings();
          },
        ),
        const Padding(
  ...
  Widget _dataManagementCard() {
    if (!_matchQuery('数据 备份 恢复 导入 导出 zip json 对话')) {
      return const SizedBox.shrink();
    }
    return _sectionCard(
      _sectionHeader('数据管理', Icons.storage_outlined, subtitle: '备份、恢复与导出导入'),
      <Widget>[
        Text('全部数据', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
        const SizedBox(height: 4),
        Text(
          '将全部数据（角色、世界、对话，不含设置）打包为 ZIP 文件，或从 ZIP 导入。',
        ...
        const Divider(),
        const SizedBox(height: 8),
        Text('仅对话', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w600)),
        const SizedBox(height: 4),
        Text(
          '导出为 JSON（可内嵌角色卡）或 Markdown，方便阅读与分享。',
          style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant),
        ),
        const SizedBox(height: 12),
        Wrap(
          ...
          children: <Widget>[
            FilledButton.tonalIcon(
              onPressed: _exportingConv || _importingConv ? null : _exportSelectedConversations,
              icon: _exportingConv
                  ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2))
            ...
            OutlinedButton.icon(
              onPressed: _exportingConv || _importingConv ? null : _importConversationsJson,
              icon: _importingConv
                  ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2))
                  : const Icon(Icons.upload_file_outlined),
  ...
  Widget _advancedCard() {
    if (!_matchQuery('高级 命令 清除 危险')) {
      return const SizedBox.shrink();
    }
    return _sectionCard(
      _sectionHeader('高级', Icons.warning_amber_outlined, subtitle: '请谨慎操作'),
      <Widget>[
        Container(
            ...
            children: <Widget>[
              Row(
                children: <Widget>[
                  ...
                  const SizedBox(width: 6),
                  Text('危险操作', style: TextStyle(fontWeight: FontWeight.w600, color: Theme.of(context).colorScheme.error)),
                ],
              ),
              const SizedBox(height: 8),
              Text(
                '输入命令并执行。命令大小写敏感。请确保您清楚自己在做什么。',
                style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant),
              ),
